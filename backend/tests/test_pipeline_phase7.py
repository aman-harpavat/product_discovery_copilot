...
def test_analyze_feedback_builds_multiple_clusters_and_preserves_mixed_signals(monkeypatch) -> None:
    ...
    sample_google_play = [
        ...
        normalize_google_play_review(
            {
                "reviewId": "gp_2",
                "content": "Discover Weekly repeats the same songs and needs more variety.",
                "score": 2,
                "thumbsUpCount": 1,
    ...
    app_store_positive_entry = ElementTree.fromstring(
    ...
    assert app_store_positive_entry is not None
    app_store_mixed_entry = ElementTree.fromstring(
        """<feed xmlns="http://www.w3.org/2005/Atom" xmlns:im="http://itunes.apple.com/rss">
        <entry>
          <id>https://itunes.apple.com/us/review?id=324684580&amp;reviewId=1002</id>
          <title>I love Spotify but recommendations need more control</title>
          <content>The app is great, but I wish recommendations had more variety.</content>
          <updated>2026-02-11T12:00:00-07:00</updated>
          <link>https://itunes.apple.com/us/review?id=324684580&amp;reviewId=1002</link>
          <im:rating>4</im:rating>
          <im:version>9.2.0</im:version>
        </entry>
        </feed>"""
    ).find("atom:entry", APP_STORE_NS)
    assert app_store_mixed_entry is not None
    sample_app_store = [
        normalize_app_store_entry(app_store_positive_entry),
        normalize_app_store_entry(app_store_mixed_entry),
    ]
